...
public class Sprite {
	...
	public void drawLine(Point p1, Point p2, int color) {
		...
		if(dx == 0 && dy == 0) {
			Starter.doAction(new SetPixelAction(p1.x, p1.y, color));
		} else if(Math.abs(dx) > Math.abs(dy)) {
			for(int i = 0; i <= dx_abs; i++) {
				Starter.doAction(new SetPixelAction(p1.x + i * dx_unit, p1.y + (i * dy) / dx_abs, color));
			}
		} else {
			for(int i = 0; i <= dy_abs; i++) {
				Starter.doAction(new SetPixelAction(p1.x + (i * dx) / dy_abs, p1.y + i * dy_unit, color));
			}
		}
	}
